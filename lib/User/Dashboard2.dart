...
//   ));
// }
class MyDashboardsub1view extends StatefulWidget
{
String? text;
String? index;
MyDashboardsub1view({required this.text,required this.index});
  @override
  createState()=>MyDashboardview1();
}
class MyDashboardview1 extends State<MyDashboardsub1view>
{
    ...
    if (response.statusCode == 200) {
     ...
     var msg1 = ed.decryptAES(xmldata.toString());
      ...
      for (var res in responseData) {
        setState(() {
               firstItems.add(MenuItem(text: res["p_year"].toString(), icon: Icons.open_in_browser,pos: 1,value:res["p_year"].toString() ));
     ...
     //
     //  }
      print("object_12_"+firstItems.length.toString());
...
//List<MenuItem>? firstItems;
    ...
    setState(() {
      firstItems1 = [MenuItem(text: responseData["self"]["NAME"].toString(), icon: Icons.download_for_offline_outlined,pos: int.parse(responseData["self"]["CARD_NO"].toString()))];
    ...
    for(int h=0;h<kk.length;h++)
    {
      c=c+h;
      setState(() {
        firstItems1.add(MenuItem(text: kk[h]["NAME"].toString(), icon: Icons.download_for_offline_outlined,pos: int.parse(kk[h]["CARD_NO"].toString())));
...
abstract class MenuItems2 {
  ...
  static const settings = MenuItem(text: '', icon: Icons.open_in_new,pos: 3);
