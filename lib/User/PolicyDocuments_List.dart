...
{
  runApp(MaterialApp(
    home: MyPolicyDocumentsListview(),
  ));
}
class MyPolicyDocumentsListview extends StatefulWidget
{
...
class MyDashboardview1 extends State
{
  ...
  Future<List<PolicyDocumentsList>> getRequest() async {


                                EncryptData ed=new EncryptData();
                            var kl=    ed.encryptData('{"method" : "policyDocument", "userid" : "Puserid"}'.replaceAll("Puserid", UserSharedPreferences.getUsername().toString()));
    ...
    '''.toString().replaceAll("paasinfotech", kl);
                            var response = await http.post(Uri.parse(link),
                                ...
                                List<PolicyDocumentsList> users = [];
                            ...
                            if (response.statusCode == 200) {
                             ...
                             for (var res in json.decode(responseData)) {
                                PolicyDocumentsList user = PolicyDocumentsList(
                                    slno: res["slno"],
                                    subject: res["subject"],
                                    file: res["file"],
